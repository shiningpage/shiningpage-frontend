...
class More extends Component<MoreProps> {
    render() {
        ...
        return (
            <div
                className={`center items-center C${fc} btnShadow min-w-[100px] h-[30px] text-centermy-[40px] mb-[30px] px-[10px] rounded-full border-[3px]
                    ${[11].includes(fc)
                        ? 'border-black/30'
                        : 'border-white/50'}
                    ${lightColors.includes(fc)
                        ? 'text-black/60'
                        : 'text-white'}
                `}
            >
