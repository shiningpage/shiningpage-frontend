...
class WebPage extends Component {
    ...
    handleScroll = async () => {
        ...
        const attachmentsSubTop = attachmentsSub.getBoundingClientRect().top;
            ...
            <div id='aboutInfo' style={{padding:'70px 0px', fontSize:w<s ? '16px' : '18px', fontWeight:100, backgroundColor:`${colors[`C${fc===11 ? 0 : fc}`]}00`}}>
                <Container className='center' style={{alignItems:'center', flexDirection:'column'}}>
                    <div style={{position:'relative'}}>
                        {userX}
                        {me && jobSummary && <EditBtn size={35} top={23} right={-50} padding='4px' onClick={() => this.onToggleActivitySummary()}/>}
                    </div>
                    <div className={`C${fc===11 ? 7 : fc}`} style={{width:'200px', height:'2px', margin:'10px 0px 30px'}}></div>
                    <div className='d-flex'>
                        {jobSummary &&
                            <div style={{whiteSpace:'pre-wrap', maxWidth:'600px', textAlign:'center', lineHeight:'30px', marginBottom:w<s ? '50px' : '70px'}}>
                    ...
                    <div className='d-flex' style={{flexDirection:w<s ? 'column' : ''}}>
                        <div style={{minWidth: w<s ? '100%' : '300px', marginRight: w<s ? 0 : '70px'}}>
                            { profileData &&
                                <img
                                    className=''
                                    style={{objectFit:'cover', height:'300px', width:'100%', borderRadius:'0px 100px', marginBottom:w<s ? '50px' : 0, border:'1px solid #99999930'}}
                                    src={aboutImgSrc}
