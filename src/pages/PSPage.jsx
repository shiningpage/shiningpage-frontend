...
import { serverURL, s, designedByColors, listRefreshQty, listRefreshQtySmall, googleAds } from '../srcSet';
...
class PSPage extends Component {
    ...
    async componentDidMount() {
        ...
        await this.props.dispatch(setPageName('ps'))
        await this.props.dispatch(setAdsInfo([]))
        await this.props.dispatch(setSubUserInfo([]))
        await this.getProductService()
        await this.props.dispatch(setSubject(`ps-${this.props.adsInfo.adsTitle}`))
    ...
    getAdsInfo = async (p1) => {
        window.scrollTo(0, 0);
        await axios.get(`${serverURL}/ads/getAdsInfo/` + p1)
        .then(async(res) => {
            var data = res.data
            ...
            (item, i) => {
                ...
                const adsTitle = (
                    <div className='d-flex' style={{width:'100%', marginBottom:'5px', fontSize:'13px', fontWeight:'', overflow: 'scroll', textAlign:'right', whiteSpace:'nowrap'}}>
                        {TLen>40 ? item.adsTitle.substr(0, 35) + ' ...' : item.adsTitle}
                    </div>
                )
                const adsPrice = (
    ...
    render() {
        ...
        const {rtl, lang, setLT, adsInfo, businessType, isAuthenticated, mainUser, subUserInfo, userImg, genderValue, userId, mainUserId, userType,
 ...
 */}                        <div className={`d-flex ${w<s ? '' : 'typewriter'}`} style={{flexDirection:'column'}}>
        ...
        const mainAds = (
            <div id='aboutInfo' style={{backgroundImage:`url(${aboutImgSrc})`, backgroundSize: 'cover', backgroundPosition: 'right', position:'relative'}}>
                ...
                <div style={{width:'100%', height:'100%', padding:'0px 0px', backgroundColor:'#ffffff99'}}>
                    <div style={{padding:w<s ? '50px 0px' : '70px 0px', fontSize:w<s ? '16px' : '16px'}}>
                        <Container className='center' style={{alignItems:'center', flexDirection:'column'}}>
                            ...
                            <div className='d-flex' style={{flexDirection:w<s ? 'column' : ''}}>
                                ...
                                {/* <strong>
                                    <h2>{adsInfo.adsTitle}</h2>
                                    <div style={{margin:'20px 0px', whiteSpace:'pre-wrap'}} dangerouslySetInnerHTML={{ __html: adsInfo.adsComment }}/>
                                </strong> */}
                            </div>
                        </Container>
                    </div>
    ...
    // console.log(advertiserId, mainUserId)
        ...
        <div style={{width:'100%', position:'relative'}}>
            <QRCodeX size="180px" url={`https://shiningpage.com/${subUserInfo.businessType>0 ? 'publisher' : 'user'}/${subUserInfo.username}`}/>
        ...
        return (
            <div>
                ...
                {addressbar}
                {1===1 && <PsCarousel adsInfo={adsInfo} fc={fc}/>}
                {googleAds && adsBox1}
                {mainAds}
                {googleAds && adsBox2}
                <Container>
...
const mapStateToProps = (state) => {
  ...
  export default connect (mapStateToProps)(PSPage);
