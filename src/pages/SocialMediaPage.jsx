...
class SocialMediaPage extends Component{
    ...
    deactivateServices = async (services) => {
        console.log('service stopted: ', services)
        ...
        //             console.log(i + 1, ' of ', services.length)
    ...
    render () {
        ...
        const orderList = (
            <div style={{ width:'100%', }}>
                <div className='d-flex'>
                    <div className='text-white' style={{ fontWeight:'450', marginBottom:'10px' }}>Orders: {SMRecordsLoading ? loader13 : SMRecords.length}</div>
                    {SMRecords.length>0 && <span className='goldenText hover:underline' style={{margin:'0px 20px', color:'blue', display: SMRecordsLoading ? 'none' : '', cursor:'pointer'}} onClick={() => this.updateOrders()}>{ ordersUpdating ? <span className='goldenText'>{loader13}</span> : 'Update' }</span>}
                </div>
                { SMRecords.length>0 &&
                    <div className=''>
                        <LocalTable
...
//+ '$' + service.rate 
		...
		const noAccess = (
			<div className='center'>
                <div className="w-[200px] text-center text-white px-[15px] py-[10px] rounded-full border !border-[#F5B942]">
        ...
        return (
            <div className='' style= {{marginBottom:'50px'}}>
                <div>
                    ...
                    { socialMediaIndex
                        ?
                        <div className={`${w<s ? 'p-[10px]' : 'p-[40px]'}`}>
